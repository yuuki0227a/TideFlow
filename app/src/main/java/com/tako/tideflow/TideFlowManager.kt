...
class TideFlowManager {
    companion object {
        const val HOURLY_TIDE_HOUR_00 = 0
        const val HOURLY_TIDE_HOUR_01 = 1
        const val HOURLY_TIDE_HOUR_02 = 2
        const val HOURLY_TIDE_HOUR_03 = 3
        const val HOURLY_TIDE_HOUR_04 = 4
        const val HOURLY_TIDE_HOUR_05 = 5
        const val HOURLY_TIDE_HOUR_06 = 6
        const val HOURLY_TIDE_HOUR_07 = 7
        const val HOURLY_TIDE_HOUR_08 = 8
        const val HOURLY_TIDE_HOUR_09 = 9
        const val HOURLY_TIDE_HOUR_10 = 10
        const val HOURLY_TIDE_HOUR_11 = 11
        const val HOURLY_TIDE_HOUR_12 = 12
        const val HOURLY_TIDE_HOUR_13 = 13
        const val HOURLY_TIDE_HOUR_14 = 14
        const val HOURLY_TIDE_HOUR_15 = 15
        const val HOURLY_TIDE_HOUR_16 = 16
        const val HOURLY_TIDE_HOUR_17 = 17
        const val HOURLY_TIDE_HOUR_18 = 18
        const val HOURLY_TIDE_HOUR_19 = 19
        const val HOURLY_TIDE_HOUR_20 = 20
        const val HOURLY_TIDE_HOUR_21 = 21
        const val HOURLY_TIDE_HOUR_22 = 22
        const val HOURLY_TIDE_HOUR_23 = 23
        const val TIDE_DATE_YEAR = 0
        const val TIDE_DATE_MONTH = 1
        const val TIDE_DATE_DAY = 2
        // %s:年(yyyy) %s:観測地点(OS)
        const val TIDE_DATA_TXT_URL_FORMAT = "https://www.data.jma.go.jp/kaiyou/data/db/tide/suisan/txt/%s/%s.txt"
        /* データ範囲の定義 */
        const val HOURLY_TIDE_LEVELS_START = 0
        const val HOURLY_TIDE_LEVELS_END = 72
        const val TIDE_DATE_START = HOURLY_TIDE_LEVELS_END
        const val TIDE_DATE_END = 78
        const val LOCATION_NAME_START = TIDE_DATE_END
        const val LOCATION_NAME_END = 80
        const val HIGH_TIDE_TIMES_START = LOCATION_NAME_END
        const val HIGH_TIDE_TIMES_END = 108
        const val LOW_TIDE_TIMES_START = HIGH_TIDE_TIMES_END
        const val LOW_TIDE_TIMES_END = 136
        /* 固定長の定義 */
        // 時間毎の潮位の列数の固定長
        const val HOURLY_TIDE_FIXED_COLUMNS = 3
        // 日付の列数の固定長
        const val TIDE_DATE_FIXED_COLUMNS = 2
        // 観測地点の列数の固定長
        const val LOCATION_NAME_FIXED_COLUMNS = 2
        // 満潮の列数の固定長
        const val HIGH_TIDE_TIMES_FIXED_COLUMNS = 7
        // 干潮の列数の固定長
        const val LOW_TIDE_TIMES_FIXED_COLUMNS = 7
        // 満潮時間の列数の固定長
        const val HIGH_TIME_FIXED_COLUMNS = 4
        // 満潮の列数の固定長
        const val HIGH_TIDE_LEVEL_TIME_FIXED_COLUMNS = 3
        // 干潮時間の列数の固定長
        const val LOW_TIME_FIXED_COLUMNS = 4
        // 干潮の列数の固定長
        const val LOW_TIDE_LEVEL_TIME_FIXED_COLUMNS = 3
        // 日付データの個数
        const val DATE_TIMES_MAX = 3
        // 満潮データの個数
        const val HIGH_TIDE_TIMES_MAX = 4
        // 干潮データの個数
        const val LOW_TIDE_TIMES_MAX = 4
        // レコードの固定文字数
        ...
        thread {
            ...
            try {
                ...
                if (response.isSuccessful && responseData != null) {
                    callback.onDataFetched(responseData)
                } else {
                    callback.onError(IOException("Failed to fetch data"))
                }
            } catch (e: IOException) {
                callback.onError(e)
            }
